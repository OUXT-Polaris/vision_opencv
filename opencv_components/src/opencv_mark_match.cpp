...
void OpenCVMatchComponent::call_back(const sensor_msgs::msg::Image::SharedPtr image_msg){
  ...
  std::vector<std::vector<cv::Point> > triangle_contours,circle_contours,cross_contours,contours,matched_contours,selected_contours;
  ...
  cv::findContours(temp, triangle_contours, triangle_hierarchy,  cv::RETR_TREE, cv::CHAIN_APPROX_SIMPLE);
